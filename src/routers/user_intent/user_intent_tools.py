...
class SearchTasksParameters(BaseModel):
    ...
    due_before: Optional[datetime] = Field(None, description="Filter tasks that are due before a specific date")
    status: Optional[str] = Field(None, description="Filter tasks by their current status", enum=["pending", "completed", "in-progress"])

class SearchTasksFunction(BaseModel):
    name: str = Field("search_tasks", description="Search for tasks based on a keyword or specific attributes")
...
class EditTaskParameters(BaseModel):
    ...
    new_status: Optional[str] = Field(None, description="The updated status of the task", enum=["pending", "completed", "in-progress"])
